...
def create_session_count(session_id):
    ...
    stmt = db.select(SessionCount).filter_by(bird_id=body_data.get("bird_id"), session_id=session_id)
...
def edit_session_count(session_id, id):
    ...
    if session_count.session_id != session_id:
        return{"Error": f"The session with id: {session_id} is not associated with the count you wish to edit. Please check your id numbers and try again."}
    ...
    if bird_id:
        ...
        stmt = db.select(db.func.count()).select_from(SessionCount).filter_by(bird_id=bird_id, session_id=session_id)
